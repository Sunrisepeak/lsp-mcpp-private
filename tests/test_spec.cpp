...
int main(int argc, char* argv[]) {
    ...
    "the level 3 GCC example loads"_test = [&] {
        auto database = mcppls::spec::load_database(base::join_path(root, "specs/examples/s1-level3-gcc.json"));
    ...
    "a database round-trips through JSON"_test = [&] {
        auto first = mcppls::spec::load_database(base::join_path(root, "specs/examples/s1-level2-clang-two-sets.json"));
    ...
    "module names resolve in the specified order"_test = [&] {
        auto database = mcppls::spec::load_database(base::join_path(root, "specs/examples/s1-level2-clang-two-sets.json"));
    ...
    "discovery output is interpreted"_test = [&] {
        ...
        const Json request = mcppls::spec::make_discovery_request({ "/w", { "/w/a.cppm" }, "debug" });
