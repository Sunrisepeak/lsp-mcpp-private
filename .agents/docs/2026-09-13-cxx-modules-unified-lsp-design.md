...
| K15 | 需求 | 无法降低子进程的调度优先级：clangd 的模块构建与后台索引与编辑器等前台程序同等优先级运行，大工程冷启动时占满多个核心 | 规范范围：openkal.process 没有进程优先级接口（对应 POSIX `setpriority`、Windows `SetPriorityClass`、macOS 任务 QoS） | 只记录；服务端以 clangd 的 `-j`（物理核数的 1/4）与模块预热并发限制占用（`.agents/docs/2026-09-15-mcppls-robustness-design.md` C7），见 issue #2 |
