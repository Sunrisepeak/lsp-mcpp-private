...
`lsp-mcpp-lspgen generate` turns it into `src/lsp/protocol.cppm` and
`src/lsp/protocol.cpp`. CI regenerates both files and fails when they differ
from the committed ones.

```bash
mcpp run lsp-mcpp-lspgen -- generate --meta-model tools/lspgen/metaModel-3.18.json --out src/lsp
```
