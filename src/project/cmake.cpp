...
base::Result<InferredDatabase> load_cmake(const Detection& detection, std::string_view privateBuildDirectory, const ProviderContext& context) {
    ...
    platform::SpawnOptions versionQuery;
    versionQuery.program = *cmake;
    versionQuery.arguments = { "--version" };
    versionQuery.workDirectory = detection.root;
    const auto versionAnswer = platform::run(std::move(versionQuery), std::chrono::seconds { 20 });
    std::optional<std::string> gateUuid;
    if (versionAnswer && !versionAnswer->timedOut && versionAnswer->exitCode == 0) gateUuid = build_database_gate_uuid(versionAnswer->output);
    ...
    options.arguments = cmake_configure_arguments(detection.root, buildDirectory, useNinja, gateUuid);
    ...
    if (wantBuildDatabase) {
        ...
        platform::SpawnOptions buildDatabaseTarget;
        buildDatabaseTarget.program = *cmake;
        buildDatabaseTarget.arguments = { "--build", buildDirectory, "--target", "build_database.json" };
        auto built = platform::run(std::move(buildDatabaseTarget), context.configureTimeout);
