...
| JSON Compilation Database (`compile_commands.json`) | Downstream format. Tools SHOULD be able to export a conforming document to it (section 12). |
...
| WG21 P3286 / EcoStd RFC #3, module metadata | Files referenced by `module-metadata` use this format. Standard library modules MUST be discoverable through it. |
| CPS 0.15 | The file a CPS package names in `cpp_module_metadata` MAY be listed directly in `module-metadata`. |
| WG21 P2717 / EcoStd RFC #2, tool introspection | Toolchain objects SHOULD be filled from tool introspection where the tool supports it. |
...
|---|---|---|---|
| `version` | integer | MUST | Format version. This profile uses `1`. |
| `revision` | integer | MUST | Format revision. This profile uses `0`. |
| `sets` | Set[] | MUST | The sets of translation units (section 7). Set names MUST be unique within a document. |
| `ide` | object | level 2 MUST | Document-level profile data (section 5.1). |

### 5.1 Document-level `ide` object

| Field | Type | Requirement | Description |
|---|---|---|---|
| `profile-version` | string | MUST | The version of this profile the document conforms to, as a semantic version, for example `"0.2.0"`. |
| `generator` | object | SHOULD | The producer: `name` (string, MUST) and `version` (string, SHOULD). |
| `toolchains` | object | MUST | Map from toolchain id to Toolchain object (section 6). Ids are opaque strings. |
...
| Field | Type | Requirement | Description |
|---|---|---|---|
| `family` | enum | MUST | `gcc`, `clang`, `msvc`, `clang-cl` or `other`. Selects the argument dialect. |
| `version` | string | MUST | The version the compiler reports. |
...
| Field | Type | Requirement | Description |
|---|---|---|---|
| `name` | string | MUST | Unique name of the set within the document. |
| `family-name` | string | SHOULD | Name shared by sets that are variants of the same target, for example the same target in two configurations. |
| `visible-sets` | string[] | MUST | Names of the sets whose non-private modules units of this set may import. MUST list the complete visibility closure (section 10). |
| `baseline-arguments` | string[] | SHOULD | Arguments common to every unit of the set, in the dialect of the set's toolchain. |
| `translation-units` | TranslationUnit[] | MUST | The units of the set (section 8). |
| `ide` | object | level 2 MUST | Set-level profile data (section 7.1). |

### 7.1 Set-level `ide` object

| Field | Type | Requirement | Description |
|---|---|---|---|
| `toolchain` | string | MUST | Id of an entry in the document-level `toolchains` map. |
| `configuration` | string | SHOULD | Configuration name, for example `debug`, `release` or `dev`. |
| `kind` | enum | SHOULD | `library`, `executable`, `test` or `other`. Consumers use it to choose a default context. |
| `options` | SemanticOptions | level 3 MUST | Structured form of `baseline-arguments` (section 9). |
...
| Field | Type | Requirement | Description |
|---|---|---|---|
| `source` | string | MUST | Path of the source file. |
| `work-directory` | string | MUST | Absolute path of the directory the compiler runs in. |
| `arguments` | string[] | MUST | The complete command line, driver first, in the dialect of the set's toolchain. |
| `local-arguments` | string[] | SHOULD | Arguments of this unit that are not in the set's `baseline-arguments`. |
| `object` | string | MAY | Path of the object file the build writes. |
| `private` | boolean | SHOULD | `true` when the modules this unit provides are not visible to other sets. Absent means `false`. |
| `provides` | object | level 1 MUST for module units | Map from each module name the unit provides to the path of the BMI the build writes for it. A producer that performs no build MAY use an empty string. |
| `requires` | string[] | level 1 MUST for units that import | Module names the unit imports, with partitions written in full (`M:P`). |
| `ide` | object | level 2 MUST | Unit-level profile data (section 8.1). |

### 8.1 Unit-level `ide` object

| Field | Type | Requirement | Description |
|---|---|---|---|
| `role` | enum | MUST | The unit's role (section 8.2). |
...
A producer **MUST** determine `role` from the source content, not from the file extension. A producer that cannot decide **MUST** write `unknown` rather than guess. A consumer that reads `unknown` **SHOULD** scan the source itself and report that it is working in a degraded mode (section 11.2).

## 9. SemanticOptions object

SemanticOptions describe, independently of any compiler dialect, the options that affect parsing, semantics and BMI compatibility. Options that do not affect meaning — optimization level, debug information, output paths, dependency file generation and most warnings — **SHOULD NOT** appear.
...
1. When `options` is present a consumer **MUST** use it. When it is absent a consumer **MUST** parse `arguments` according to `toolchain.family`.
...
3. A producer **MUST NOT** place BMI location arguments (a module mapper, `-fmodule-file=`, `/reference` and similar) in `options`.
...
- A producer **MUST** list the complete visibility closure in `visible-sets`. A consumer **MUST NOT** derive visibility transitively.
- When one step finds more than one provider, a consumer **MUST** report an ambiguity diagnostic and **MAY** continue with the first provider in document order.
- When no step finds a provider, a consumer **MUST** report an unresolved-module diagnostic at the corresponding `import` declaration.
- A partition `M:P` is importable only by units of module `M`. A consumer **SHOULD** diagnose an import of another module's partition.
...
- **MUST** ignore unknown fields and unknown extensions;
- **MUST NOT** require that the BMI paths in `provides` exist;
- **MUST NOT** use build BMIs for semantic analysis unless all of the following hold: the consumer's semantic engine matches the toolchain's `family`, `version` and `build-id` exactly; the user has explicitly enabled an authoritative mode; and the consumer can detect that a BMI is stale relative to its sources;
- **MUST** interpret `arguments` in the dialect of `toolchain.family`;
- **MUST** resolve modules and report unresolved and ambiguous names as specified in section 10;
- **SHOULD** accept level 1 documents, inferring `role` by lexical scanning and completing toolchain information by querying the driver, and **SHOULD** tell the user that it is working in a degraded mode.

## 12. Export to `compile_commands.json`

So that tools that do not implement this profile keep working, producers and conversion tools **SHOULD** offer an export:
...
- When the same source file appears in more than one set, the exporter **MUST** either let the caller choose the set or export only the entries of the default context.
- The export loses the module graph, toolchain and role information. The exporter **SHOULD** say so in its documentation or log.
...
- Toolchain queries and discovery commands execute external programs. A consumer **MUST** run them only in a trusted workspace and **SHOULD** restrict driver queries to an allow-list, in the manner of clangd's `--query-driver`.
- Standard library module sources usually lie outside the workspace. A consumer **MUST** access them read-only.
- Paths in a database may point anywhere. A consumer **MUST NOT** write to a location because a database names it.
