...
correctly (that's [conformance/](../conformance/)). Design doc
[.agents/docs/2026-09-15-mcppls-overall-design.md](../.agents/docs/2026-09-15-mcppls-overall-design.md)
§10.2, work item A2.

```bash
python3 bench/run.py validate                       # no agent, no network, no API key — CI runs this
python3 bench/run.py run --agent claude-code --arm mcppls-lsp \
...
- `check` — a list of commands (each an argv list, run with the project copy as the working directory, no shell).
  Every command must exit 0 for the task to count as solved; they run like a `&&` chain (stop at the first
  failure). Every task's checks are `mcpp build`, then `mcpp test`, then (most tasks) a `python3 -c ...`
  one-liner that greps the changed source file for a substring the fix must (or must no longer) contain — a
  content check, not a second build, so it stays fast and has no new dependency.
...
`bench/run.py`'s `apply_patch()` expects — see its module docstring):
...
Verify with `python3 bench/run.py validate --only <id>` before committing — never build or test directly
inside `bench/tasks/<id>/project/` itself, since that leaves `target/` and `compile_commands.json` behind
(`validate`'s own project copies clean these on the way in, but the committed source should never carry them).

## `validate`

```
python3 bench/run.py validate [--tasks DIR] [--only ID] [--timeout SECONDS]
```

For every task: copies `project/` to a temporary directory and runs `check` (expects at least one command to
fail); copies `project/` again, applies `reference.patch` (`git apply -p2`, falling back to a small
pure-Python unified-diff applier if `git` is missing or refuses the patch — both expect the `a/a/...`,
`b/b/...` paths the authoring recipe above produces), and runs `check` again (expects every command to
succeed). Prints one `PASS`/`FAIL` line per task and exits non-zero if any task fails either half. This is
what `.github/workflows/ci.yml`'s `bench-validate` job runs — no agent, no network beyond what `mcpp
build`/`mcpp test` already need (toolchains are pre-installed by `setup-mcpp`), no API key.

## `run` — spends API credits; never run automatically

```
python3 bench/run.py run --agent claude-code|copilot-cli --arm grep|clangd-lsp|mcppls-lsp|mcppls-mcp \
...
- GitHub Copilot CLI's docs describe no JSON output flag at all, so `turns`/`input_tokens`/`output_tokens`/
  ...
  `files_changed_outside_scope` still come from `run.py`'s own measurements, not the agent's output.
